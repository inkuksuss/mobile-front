...
export function loginRequest(client: AxiosInstance, form: LoginForm): Promise<ResultResponse> {
  const promiseFn = (
...
export function joinRequest(client: AxiosInstance, form: AddMemberForm): Promise<ResultResponse> {
  const promiseFn = (
    fnResolve: (value: PromiseLike<ResultResponse> | ResultResponse) => void,
    fnReject: (reason?: any) => void,
  ) => {
